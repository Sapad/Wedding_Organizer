...
class Katalog extends CI_Controller 
{
	...
	public function edit()
	{
		if ($this->input->get('id')) {
			...
			if ($cek_data > 0) { 
            	...
            	role="alert"><strong>Upss </strong>Data tidak tersedia!<i class="remove ti-close" data-dismiss="alert"></i></div>');
	...
	public function addData()
    {
        ...
        $data = array(
            ...
            if ($insert) {
                ...
                role="alert"><strong>Upss! </strong>Data Gagal Di Simpan! <i class="remove ti-close" data-dismiss="alert"></i></div>');
                redirect('admin/Katalog');
            	}
    		} else {
        	redirect('admin/Katalog');
	...
	public function updateData()
    {
		...
		if ($cek_data > 0) { 
            ...
            if ($update) {
                ...
                $this->session->set_flashdata('message', '<div class="alert alert-danger alert-dismissible fade show" 
                role="alert"><strong>Upss! </strong>Data Gagal di Ubah! <i class="remove ti-close" data-dismiss="alert"></i></div>');
                redirect('admin/Katalog');
            	}
			}
    	} else {
	...
	public function delete()
	{
		if (!empty($this->input->get('id', true))) {
			...
			if ($delete) {
                ...
                $this->session->set_flashdata('message', '<div class="alert alert-danger alert-dismissible fade show" 
                role="alert"><strong>Upss! </strong>Data Gagal di Hapus! <i class="remove ti-close" data-dismiss="alert"></i></div>');
                redirect('admin/Katalog');
            }
    	} else {
        	redirect('admin/Katalog');
	...
	private function _do_upload($fileName) 
    {
        ...
        if (!$this->upload->do_upload('image')) //upload and validate
        {
            ...
            $data['error_string'][] = 'Upload error :' . $this->upload->display_error('', ''); //show ajax error
            $data['status'] = FALSE;
